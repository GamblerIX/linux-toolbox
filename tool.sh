...
function show_header() {
    ...
    echo -e "${PURPLE}  检测到系统: ${OS_TYPE} | 版本: v${VERSION}${NC}"
}
...
function website_analysis() {
    show_header
    echo -e "${YELLOW}====== 网站日志分析 (开发中) ======${NC}"
    echo -e "\n此功能正在开发中，敬请期待！"
    echo -e "未来将支持：自动查找并分析指定网站的Nginx日志，"
    echo -e "快速定位高频访问IP、404/500错误等。"
    read -p "按回车键返回..." < /dev/tty
}

function panel_enhancement_menu() {
    show_header
    echo -e "${YELLOW}====== 1. 面板增强工具 ======${NC}"
    echo -e "${GREEN}1. 网站日志分析 ${NC}"
    echo -e "${GREEN}0. 返回主菜单 ${NC}"
    echo -e "${CYAN}==============================================${NC}"
    
    read -p "请输入选项 [0-1]: " choice < /dev/tty
    case $choice in
        1) website_analysis; panel_enhancement_menu ;;
        0) main_menu ;;
        *) echo -e "${RED}无效选项${NC}"; sleep 1; panel_enhancement_menu ;;
    esac
}
...
function core_system_management_menu() {
    show_header
    echo -e "${YELLOW}====== 2. 系统核心管理 ======${NC}"
    echo -e "${GREEN}1. 用户管理 ${NC}"
    echo -e "${GREEN}2. 内核管理 ${NC}"
    echo -e "${GREEN}3. 清理系统垃圾 ${NC}"
    echo -e "${GREEN}0. 返回主菜单 ${NC}"
    echo -e "${CYAN}==============================================${NC}"
    
    read -p "请输入选项 [0-3]: " choice < /dev/tty
    case $choice in
        1) user_management; core_system_management_menu ;;
        2) kernel_management; core_system_management_menu ;;
        3) clean_system; read -p "按回车键返回..." < /dev/tty; core_system_management_menu ;;
        0) main_menu ;;
        *) echo -e "${RED}无效选项${NC}"; sleep 1; core_system_management_menu ;;
    esac
}
...
function firewall_management() {
    ...
    case $choice in
        ...
        0) network_and_security_menu; return ;;
        *) echo -e "${RED}无效选项${NC}"; sleep 1 ;;
    esac
...
function network_and_security_menu() {
    show_header
    echo -e "${YELLOW}====== 3. 网络与安全 ======${NC}"
    ...
    case $choice in
        ...
        5) list_used_ports ;;
        0) main_menu ;;
        *) echo -e "${RED}无效选项${NC}"; sleep 1; network_and_security_menu ;;
    esac
}
...
function software_and_installation_menu() {
    show_header
    echo -e "${YELLOW}====== 4. 软件源与面板安装 ======${NC}"
    echo -e "${GREEN}1. 一键换源加速 ${NC}"
    echo -e "${GREEN}2. 一键安装面板 ${NC}"
    echo -e "${GREEN}3. 一键安装singbox-yg脚本 ${NC}"
    echo -e "${GREEN}0. 返回主菜单 ${NC}"
    echo -e "${CYAN}==============================================${NC}"

    read -p "请输入选项 [0-3]: " choice < /dev/tty
    case $choice in
        1) change_source; software_and_installation_menu ;;
        2) panel_installation; software_and_installation_menu ;;
        3) install_singbox_yg ;;
        0) main_menu ;;
        *) echo -e "${RED}无效选项${NC}"; sleep 1; software_and_installation_menu ;;
    esac
}

function show_changelog() {
    show_header
    echo -e "${YELLOW}====== 版本与更新日志 ======${NC}"
    echo -e "当前版本: ${GREEN}v${VERSION}${NC}"
    echo -e "\n--- 更新历史 ---"
    echo -e "${YELLOW}v2.0 (2025-06-15):${NC}"
    echo -e "  - ${GREEN}[重构]${NC} 全面重构菜单结构，逻辑更清晰。"
    echo -e "  - ${GREEN}[新增]${NC} 增加“面板增强工具”分类，为未来功能做准备。"
    echo -e "  - ${GREEN}[修复]${NC} 彻底修复用户管理模块的交互逻辑和过滤Bug。"
    echo -e "  - ${GREEN}[优化]${NC} 调整多个菜单的返回逻辑，优化操作流程。"
    echo -e "\n${YELLOW}v1.x (历史版本):${NC}"
    echo -e "  - 初始版本发布与多项功能迭代。"
    read -p "按回车键返回..." < /dev/tty
}

function toolbox_management() {
    show_header
    echo -e "${YELLOW}====== 5. 工具箱管理 ======${NC}"
    echo -e "${GREEN}1. 查看版本与更新日志 ${NC}"
    echo -e "${GREEN}2. 安装/更新 工具箱 ${NC}"
    echo -e "${GREEN}3. 卸载工具箱 ${NC}"
    echo -e "${GREEN}0. 返回主菜单 ${NC}"
    echo -e "${CYAN}==============================================${NC}"
    
    read -p "请输入选项 [0-3]: " choice < /dev/tty
    case $choice in
        1) show_changelog; toolbox_management ;;
        2) install_toolbox ;;
        3) uninstall_toolbox ;;
        0) main_menu ;;
...
function main_menu() {
    show_header
    echo -e "${GREEN}1. 面板增强工具 ${NC}"
    echo -e "${GREEN}2. 系统核心管理 ${NC}"
    echo -e "${GREEN}3. 网络与安全 ${NC}"
    echo -e "${GREEN}4. 软件源与面板安装 ${NC}"
    ...
    read -p "请输入选项 [0-5]: " choice < /dev/tty
    case $choice in
        1) panel_enhancement_menu ;;
        2) core_system_management_menu ;;
        3) network_and_security_menu ;;
        4) software_and_installation_menu ;;
